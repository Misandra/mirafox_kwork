...
class RatingManager {
	...
	public static function createAuto($orderId, $comment, $mode) {
		...
		KworkManager::setReviewCount($order->kworkId);

		WorkManager::add(WorkManager::TYPE_UPDATE_USER_LEVEL, $order->workerId, WorkManager::getNight(), 3, true);

		// уведомления
		{
			$data = OrderManager::getOrderData($order->id);
			TrackManager::createAdviceTrack($data, 'bad_review');
		}

		App::pdo()->execute("INSERT INTO ratings_moder 
									SET rating_id = " . mres($rid) . ",
										status = 'new'");

		// метрики пользователя на пересчет
		\UserStatisticManager::recalcUserOrderMetrics($order->workerId, null);
