...
class KworkManager {
	...
	/**
	 * функция массовой смены категорий у кворков
	 * @param array $kworksArr массив кворков
	 * @param int $newCategory id новой категории
	 */
	public static function bulkCategoryChange($kworksArr, $newCategory) {
		$params = [];
		$kworkIds = App::pdo()->arrayToStrParams($kworksArr, $params, PDO::PARAM_INT);
		$categoryName = App::pdo()->fetchScalar("SELECT name FROM categories WHERE CATID = :id", ["id" => $newCategory]);
		$names = [];
		$previousCategoryIds = [];
		foreach ($kworksArr as $kworkId) {
			$previousCategoryIds[$kworkId] = App::pdo()->fetchScalar("SELECT category FROM posts WHERE PID = :kworkId", ['kworkId' => $kworkId]);
			$names[$kworkId]["old"] = App::pdo()->fetchScalar("SELECT name FROM categories WHERE CATID = :id", ["id" => $previousCategoryIds[$kworkId]]);
			$names[$kworkId]["new"] = $categoryName;
		}
		App::pdo()->update(KworkManager::TABLE_KWORKS, ['category' => $newCategory], "PID IN ({$kworkIds})", $params);

		/** @var Kwork[] $activeKworks */
		$activeKworks = Kwork::query()
			->whereIn(Kwork::FIELD_PID, $kworksArr)
			->where(KworkManager::FIELD_ACTIVE, KworkManager::STATUS_ACTIVE)
			->where(KworkManager::FIELD_FEAT, KworkManager::FEAT_ACTIVE)
			->get();

		// Обновляем записи в таблице для поиска кворков
		foreach ($activeKworks as $kwork) {
			KworkSearchUpdate::taskReplaceRow($kwork->lang, $kwork->PID);
		}

		foreach ($kworksArr as $kworkId) {
			if ($previousCategoryIds[$kworkId] != $newCategory) {
				$kworkLink = new KworkLinkManager($kworkId);
				$kworkLink->removeAllLinks();
				WorkManager::add("kwork_links_fill", $kworkId, null, 1, true);
				$kwork = App::pdo()->fetch("SELECT PID as kworkId, gtitle as kworkName, category, USERID as userId FROM posts WHERE PID = :kworkId", ['kworkId' => $kworkId]);
				$kworkUrl = KworkManager::generateUrl($kwork['kworkId'], $kwork['kworkName'], $kwork['category']);
				App::pdo()->update(KworkManager::TABLE_KWORKS, ['url' => $kworkUrl], "PID = :kworkId", ['kworkId' => $kwork['kworkId']]);
				WorkManager::add('update_user_categories', $kwork['userId'], WorkManager::getNight(), 3, true);

				// т.к. категория изменилась, а атрибуты завязаны на категории, то удалим их у кворка
				KworkAttributeManager::deleteByKworkId($kworkId);

				self::logStatus($kworkId, null, null, self::KWORK_LOG_TYPE_ADMIN_CHANGE_CATEGORY, null, ["category" => $names[$kworkId]]);
			}
		}
		\Kwork\Cache\Cache::invalidateCategories(array_merge([$newCategory], $previousCategoryIds));
	}
