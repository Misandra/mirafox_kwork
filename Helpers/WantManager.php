...
class WantManager {
	...
	public static function setStop(int $wantId, bool $sendMail = false, bool $isAuto = false) {
		...
		if ($isAuto) {
			...
			$wantLog->action_name_id = WantLog::ACTION_NAME_SYSTEM_AUTO_STOPPED;
		} else {
			if (UserManager::isVirtual()) {
				$wantLog->action_type_id = WantLog::ACTION_TYPE_VIRTUAL;
				$wantLog->action_name_id = WantLog::ACTION_NAME_VIRTUAL_STOPPED;
				$session = SessionContainer::getSession();
				$wantLog->admin_id = $session->get("ADMINID");
			} else {
				$wantLog->action_type_id = WantLog::ACTION_TYPE_USER;
				$wantLog->action_name_id = WantLog::ACTION_NAME_USER_STOPPED;
			}
			if (!$wantLog->admin_id) {
		...
		if ($sendMail) {
			WantNotify::sendStopRequest($want->id);
		}

		$conn->Execute("DELETE FROM offer_view WHERE want_id='" . mres($want->id) . "'");
		WorkManager::clearMultiple(["expire_want", \WorkManager::TYPE_SEND_OFFERS_EMAIL], [$want->id], false);

		//Отправка уведомления о необходимости выбрать исполнителя в проекте. Если он не выбран.
		self::createNotifyForNeedSelectWorkerIfNeeded($want);
	}
	...
	/**
	 * Создать, если необходимо, уведомление о необходимости выбрать исполнителя для заказа
	 *
	 * @param Want $want
	 */
	private static function createNotifyForNeedSelectWorkerIfNeeded(Want $want) {
		// если дата создания старше порога скрытия запросов, то не будем создавать уведомление
		$dateCreateUnix = strtotime($want->date_create);
		$dateStopUnix = time() - self::PAYER_HIDE_STOPPED_THRESHOLD;
		if ($dateCreateUnix < $dateStopUnix) {
			return;
		}

		// проверим есть ли предложения
		$want->loadMissing("offers");
		if ($want->offers->isEmpty()) {
			return;
		}

		//Только офферы с момента последней активации
		$doneOffers = $want->offers
			->filter(function(Offer $offer) use ($want) {
				return $offer->date_create >= $want->date_active;
			})
			->filter(function(Offer $offer) {
				return $offer->isDone();
			});
		if ($doneOffers->isNotEmpty()) {
			return;
		}

		//Все активные, даже старые предложения
		$activeOffers = $want->offers->filter(function(Offer $offer) {
		...
		// убедимся что по запросу нет заказа
		$orderNotExist = Order::query()
			->where(Order::FIELD_USERID, $want->user_id)
			->where(Order::FIELD_PROJECT_ID, $want->id)
			->whereIn(Order::FIELD_STATUS, [OrderManager::STATUS_DONE, OrderManager::STATUS_CHECK, OrderManager::STATUS_INPROGRESS])
			->doesntExist();

		if ($orderNotExist) {
			WantNotify::needSelectWorker($want);

			// добавим задание на скрытие уведомления о необходимости выбора исполнителя
			$expireTime = strtotime($want->date_create) + self::PAYER_HIDE_STOPPED_THRESHOLD - time();
			if ($expireTime > 0) {
				$expireTime = ($expireTime > 7 * Helper::ONE_DAY) ? 7 * Helper::ONE_DAY : $expireTime;
				$dateExpire = time() + $expireTime;
				WorkManager::add(WorkManager::TYPE_WANT_READ_NOTIFY_SELECT_WORKER, $want->id, $dateExpire);
			}
		}
	}

	/**
	 * Удалить все запросы пользователя (присвоить заданный статус)
	 *
	 * @param int $userId - Идентифкатор пользователя
	 * @param string $status - Устанавливаемый статус
	 *
	 * @return int|false - Возвращает количество изменных строк. false в случае ошибки
	 */
	public static function deleteUserWants(int $userId, string $status = self::STATUS_DELETE) {
		if (!$userId || !in_array($status, [Want::STATUS_DELETE, Want::STATUS_ACTIVE, Want::STATUS_CANCEL, Want::STATUS_NEW, Want::STATUS_STOP, Want::STATUS_USER_STOP])) {
			return;
		}

		$wants = App::pdo()->fetchAll("SELECT * FROM ". self::TABLE_NAME ." WHERE ". self::F_USER_ID ." = :userId", ["userId" => $userId]);
		foreach ($wants as $want) {
			// удалим запланированные задачи по запросам пользователя
			WorkManager::clear(WorkManager::TYPE_EXPIRE_WANT, $want["id"]);
			WorkManager::clear(WorkManager::TYPE_SEND_OFFERS_EMAIL, $want["id"]);

			// удалим просмотры запросов пользователя
			self::clearViews($want["id"]);
		}

		// удалим предложения (присвоим статус `delete`) по данному запросу
		if ($status == self::STATUS_DELETE) {
			...
			Offer::query()
				...
				->update([
					Offer::FIELD_STATUS => OfferManager::STATUS_DELETE
				]);
		}

		// выбираем запросы пользователя которые не удалены
		$wantQuery = Want::query()
			->where(Want::FIELD_USER_ID, $userId)
			->where(Want::FIELD_STATUS,"!=", self::STATUS_DELETE);

		// зафиксируем изменение в WantLog
		$wantIds = $wantQuery->pluck(Want::FIELD_ID)->toArray();
		foreach ($wantIds as $wantId) {
			$wantLog = new WantLog();
			$wantLog->want_id = $wantId;
			$wantLog->status = $status;
			$wantLog->action_type_id = WantLog::ACTION_TYPE_MODER;
			$wantLog->action_name_id = WantLog::ACTION_NAME_MODER_CANCEL;
			$session = SessionContainer::getSession();
			$wantLog->admin_id = $session->get("ADMINID");
			if (!$wantLog->admin_id) {
				$wantLog->user_id = UserManager::getCurrentUserId();
		...
		// удалим запросы (присвоим заданный статус)
		$wantQuery->update([self::F_STATUS => $status]);
	...
	public static function stopUserWants(int $userId) {
		...
		if (!empty($wantIds)) {
			...
			foreach ($wantIds as $wantId) {
				$insertData[] = [
					WantLog::F_USER_ID => $userId,
					WantLog::F_WANT_ID => $wantId,
					WantLog::F_STATUS => Want::STATUS_STOP
				];
			}
			WantLog::insert($insertData);

			// удалим запланированные задачи по запросам пользователя
			$workTypes = [
				WorkManager::TYPE_EXPIRE_WANT,
				WorkManager::TYPE_WANT_WITHOUT_OFFERS_LETTER,
				WorkManager::TYPE_SEND_OFFERS_EMAIL
			];
			WorkManager::clearMultiple($workTypes, $wantIds);
		}
	}
