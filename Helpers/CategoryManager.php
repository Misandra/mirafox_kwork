...
class CategoryManager {
	 ...
	 * @param string $sort Сортировка
	 * @param array $pageData Массив данных пагинации
	 * @param null $envData Данные окружения (для тестирования)
	 * @return array
	 */
	public static function getByCategoryNew($filters, $sort, $pageData, $envData = null) {
		$manager = new \KworkSearchManager();
		return $manager->search($filters, $sort, $pageData, $envData);
	}
	...
	public static function getPriceBoundsInCategory($filters, $priceLimits = null, $priceLimitsQuery = null) : array {
		...
		if (!empty($bounds)) {
			if (empty($priceLimitsQuery)) {
				$manager = new \KworkSearchManager();
				$priceLimitsQuery = $manager->getCommonQuery($filters);
				foreach ($bounds as $key => $bound) {
					$query = clone $priceLimitsQuery;
					$manager->addPriceQuery($query, ["from" => $bound["priceFrom"] ?: null, "to" => $bound["priceTo"] ?: null]);
					if (!$query->value("ks.id")) {
						unset($bounds[$key]);
					}
				}
			} else {
