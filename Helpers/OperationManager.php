...
class OperationManager {
	...
	/**
	 * Отменить операцию выведенную через соларстафф,
	 * которая была отмечена как выполненная
	 * и вернуть средства пользователю на основной счет
	 * @param int $operationId - идентификатор операции
	 * @param string $reason - причина отмены вывода
	 * @return array [success, message]
	 * @uses WdManageController()->actionDeclineSolarDoneWithdraw()
	 */
	public static function cancelSolarDoneWithdraw($operationId, $reason) {
		$result = ["success" => false, "message" => ""];
		if (!$operationId || !$reason) {
			$result["message"] = "Не заданы данные";
			return $result;
		}
		$operation = self::get($operationId);
		if (!$operation || $operation->{self::FIELD_TYPE} !=self::FIELD_TYPE_WITHDRAW) {
			$result["message"] = "Вывод не найден";
			return $result;
		}
		if ($operation->{self::FIELD_STATUS} != self::FIELD_STATUS_DONE && $operation->wd_status != self::FIELD_WD_STATUS_CANCEL) {
			$result["message"] = "Вывод не завершен";
			return $result;
		}
		try {
			\Core\DB\DB::transaction(function () use ($operation, $reason, &$result) {
				//Обновление операции
				Operation::whereKey($operation->{self::FIELD_ID})
					->update([
						self::FIELD_DATE_DONE => Helper::now(),
						self::FIELD_STATUS => self::FIELD_STATUS_CANCEL,
					]);

				SolarOperation::whereKey($operation->{self::FIELD_ID})
					->update([
						SolarOperation::FIELD_STATUS => SolarOperation::STATUS_CANCEL,
						SolarOperation::FIELD_REASON => $reason,
					]);

				//Начисление пользователю средств на основной счет
				User::whereKey($operation->{self::FIELD_USER_ID})
					->increment(UserManager::FIELD_FUNDS, $operation->{self::FIELD_AMOUNT});

				// Удаление ворка на проверку статуса операции в Соларе
				WorkManager::clear(WorkManager::TYPE_SOLAR_STAFF_OPERATION_CHECK, $operation->{self::FIELD_ID});

				$result["success"] = true;
				$result["message"] = "Операция вывода отменена";
			}, 5);
		} catch (\Throwable $exception) {
			Log::write(__CLASS__ . "::" . __FUNCTION__ . ": " . $exception->getMessage(), "transaction");
			$result["message"] = "Ошибка при выполнении операции";
