...
class FileManager {
	...
	/**
	 * Удаление старых файлов (старше 6 месяцев)
	 *
	 * @return void
	 */
	public static function deleteOldFiles() {
		// Вместо фильтра по дате используем фильтр по ИД, здесь определяем ИД последнего неактуального по дате файла
		$lastExpiredFileId = File::select(File::FIELD_ID)
			->where(File::FIELD_TIME, "<=", Carbon\Carbon::now()->subMonths(self::RETENTION_PERIOD)->timestamp)
			->orderByDesc(File::FIELD_TIME)
			->orderBy(File::FIELD_ID)
			->value(File::FIELD_ID) ?? 0;

		$chunks = [];

		// удалим локальные старые файлы сущностей conversation и track
		File::where(File::FIELD_ID, "<=", $lastExpiredFileId)
			->where(File::FIELD_STATUS, File::STATUS_ACTIVE)
			->whereIn(File::FIELD_ENTITY_TYPE, [
				File::ENTITY_TYPE_CONVERSATION,
				File::ENTITY_TYPE_TRACK,
			])
			->orderBy(File::FIELD_ID)
			->chunk(5000, function(\Illuminate\Database\Eloquent\Collection $files) use (&$chunks, $lastExpiredFileId) {

				// получим все s файлов в данном чанке
				$fileSs = $files->pluck(File::FIELD_S);

				// получим все s, которые нельзя удалять
				$locallyUsedS = File::where(File::FIELD_ID, ">", $lastExpiredFileId)
					->where(File::FIELD_STATUS, File::STATUS_ACTIVE)
					->where(function(\Illuminate\Database\Eloquent\Builder $files) {
						$files->where(File::FIELD_STORAGE, File::STORAGE_LOCAL)
							->orWhere(File::FIELD_STORAGE, File::STORAGE_S3)
							->whereHas("cache");
					})
					->whereIn(File::FIELD_S, $fileSs)
					->pluck(File::FIELD_S);

				// пометим файлы, которые можно удалять
				$files->whereNotIn(File::FIELD_S, $locallyUsedS)->each(function(File $file) {
					$file->safeToDelete = true;
				});

				$trackIds = $files->where(File::FIELD_ENTITY_TYPE, File::ENTITY_TYPE_TRACK)
					->pluck(File::FIELD_ENTITY_ID)
					->toArray();

				$tracks = Track::query()
					->with(["order" => function(Relations\BelongsTo $query) {
						$query->addSelect(
							Order::FIELD_OID,
							Order::FIELD_HAS_STAGES,
							Order::FIELD_WORKER_ID,
							Order::FIELD_STATUS . " as order_status",
							Order::FIELD_OID . " as order_id"
						);
					}])
					->select([
						Track::FIELD_ORDER_ID,
						Track::FIELD_USER_ID,
						Track::FIELD_ID . " as track_id",
						Track::FIELD_TYPE . " as track_type"
					])
					->whereIn(Track::FIELD_ID, $trackIds)
					->get()
					->toArray();

				// Объединим поля из двух таблиц в один массив
				$orderInfo = [];
				foreach ($tracks as $track) {
					$orderInfo[$track["track_id"]] = array_merge($track, $track["order"]);
					unset($orderInfo[$track["track_id"]]["order"]);
				}

				$deletedFileIds = [];
				/** @var File $file */
				foreach ($files as $file) {
					if ($file->entity_type == File::ENTITY_TYPE_TRACK) {
						$trackId = $file->entity_id;
						if (!isset($orderInfo[$trackId])) { // по какой-то причине трек отсутствует
							continue;
						}
						$orderInfo[$trackId]["s"] = $file->s;
						$orderInfo[$trackId]["entity_type"] = $file->entity_type;
						$orderInfo[$trackId]["entity_id"] = $file->entity_id;
						if (!self::canDeleteOldFileTrack($orderInfo[$trackId])) {
							continue;
						}
					}

					// непосредственное удаление локальных файлов на диске
					if ($file->storage == File::STORAGE_LOCAL && $file->safeToDelete && file_exists($file->path)) {
						@unlink($file->path);
						$deletedFileIds[] = $file->FID;
					}

					// удаление файлов в Amazon s3
					if ($file->storage == File::STORAGE_S3 && $file->safeToDelete) {
						\WorkManager::add(\WorkManager::TYPE_FILE_S3_DELETE, $file->FID, time()+\Helper::ONE_HOUR, 4);
						$deletedFileIds[] = $file->FID;
					}
				}

				// добавим идентификаторы файлов к удалению в данном чанке
				$chunks[] = $deletedFileIds;
			});

		// "мягко" удалим файлы в таблице
		foreach ($chunks as $fileIds) {
			File::whereIn(File::FIELD_ID, $fileIds)
				->update([
					File::FIELD_STATUS => File::STATUS_DELETED,
				]);
		}
	}
