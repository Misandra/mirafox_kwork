...
function getActor() {
	...
	if ($session->isExist("USERID")) {
		...
		// пользователь зашел на кворк до срабатывания ворков блокировки из-за долгого отсутсвия
		if ($actor->live_date < time() - (Helper::ONE_YEAR + Helper::ONE_MONTH * 6) && !$actor->isVirtual) {
			WorkManager::clear(WorkManager::TYPE_KWORK_DISABLED_PART1, $actor->id);
			WorkManager::clear(WorkManager::TYPE_KWORK_DISABLED_PART2, $actor->id);
		}
